...
# Strong structured prompt with enforced JSON output
template = """
...
Return raw JSON without markdown, code fences, explanations, or greetings. Do not include any extra text before or after the JSON.
"""
...
# Utility to extract clean JSON from text output
...
# Analysis function for Streamlit backend integration
def analyze_learning_goal(student_input):
    try:
        response = chain.invoke({"student_input": student_input})
        response_text = response.get("output", str(response)).strip()
...
if __name__ == "__main__":
    student_input = "i want to learn dsa from scratch for my placement"
